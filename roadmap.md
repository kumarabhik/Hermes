...
- PowerShell smoke scripts verify deterministic synthetic replay, one-loop observe-mode daemon artifact generation, benchmark plan artifact generation, bounded benchmark workload launch, and benchmark-plus-Hermes observe-only replay through the direct `g++` path.
...
- [~] Observe-only mode can launch Hermes around a bounded workload mix, replay the resulting Hermes run, and embed replay counts plus peak fields into the benchmark summary without host mutation; real detection-quality comparison outputs are still pending.
